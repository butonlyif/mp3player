...
- Crossfade is enabled by default but can be disabled from audio settings. Disabling it uses safe direct track switches while leaving loudness balancing and section-aware visuals available.
...
The Crossfade preference is durable local UI state. Turning it off during an active transition cancels automation, keeps the louder/intended deck, and returns both deck gains to stable values.
...
- Crossfade disabled: do not schedule overlapping gain automation; preserve basic ended/next behavior.

## Testing

Test the deck state machine, active/standby promotion, equal-power curves, 1–4 second duration boundaries, manual 0.5-second transitions, disabling before and during a transition, preload failure, stale transition tokens, rapid navigation, pause/seek cancellation, repeat-one behavior, loudness gain limits, quiet-intro protection, unavailable-analysis fallbacks, section hysteresis, and smoothed visual intensity.
